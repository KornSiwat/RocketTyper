...
class World():
    ...
    def __init__(self):
        ...
        self.pressing_key = 0
        self.next_level_time = 30

    def setup_rocket(self):
        scale = 1.3
        self.rocket = Rocket(
            width=self.width, height=self.height, x=110, y=-120)
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket6.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket5.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket4.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket3.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket2.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket1.png', scale=scale))
        self.rocket.append_texture(arcade.load_texture(
            'images/rocket.png', scale=scale))

    def setup_missile_manager(self, fileName):
        rocket_body_position = self.rocket.right + 90
        self.missile_manager = MissileManager(self.width, self.height, missileTarget=rocket_body_position,
                                              wordFileName=fileName, on_missile_destroy=self.on_missile_destroy, on_missile_hit=self.on_missile_hit)
    ...
    def setup_cloud(self):
        ...
        self.cloud_list.sprite_list = list(
            map(lambda x: Cloud(self.width, self.height + 50), range(cloudAmount)))
    ...
    def draw_stat_bar(self):
        arcade.draw_text('RocketTyper', 100, 25,
    ...
    def draw_health_bar(self):
        arcade.draw_rectangle_filled(
            self.width//2, 35, 305, 55, color=arcade.color.WHITE)
        arcade.draw_rectangle_filled(self.width//2, 35/100 * self.rocket.health,
                                     305, 60 / 100 * self.rocket.health, color=arcade.color.ORANGE_RED)
        arcade.draw_text('HEALTH', self.width//2 - 50, 25,
                         arcade.color.BLACK, font_size=25)

    def draw_missile_count(self):
        arcade.draw_text(str(f'Word: {self.destroyedMissileAmount}'),
                         self.width//2 + 175, 25, arcade.color.BLACK, font_size=22)

    def draw_time(self):
        arcade.draw_text(str(f'Time: {self.playTime:.2f} s'),
                         self.width//2 + 300, 25, arcade.color.BLACK, font_size=22)
    ...
    def update_level(self):
        ...
        if self.playTime >= self.next_level_time:
            self.missile_manager.increase_missile_speed()
    ...
    def on_gameover(self):
        self.matchStatScene = MatchStatScene(
            self.destroyedMissileAmount, self.playTime)
